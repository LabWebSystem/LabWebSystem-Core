...
Production の Release、Compose、`runtime.env` の契約は [`LabWebSystem Coreデプロイ仕様書.md`](LabWebSystem%20Coreデプロイ仕様書.md) と [`LabWebSystem外部契約.md`](LabWebSystem外部契約.md) を参照してください。デプロイを開始する標準入口は `mise run deploy` です。
